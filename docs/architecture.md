...
Persona Diversity Validator ─ services/persona/diversity.py
   ...
   ▼
Multi-Criteria Persona Evaluation ─ services/inference/ (mock | nvidia | vllm)
...
Aggregator / Report Builder ── services/aggregation/report_builder.py
   ...
   ▼
Analyst / Report Model ─────── services/analyst/ (mock | nvidia)
   │   re-narrates summary/recommendations/objection-labels/kill-quote from the
   │   already-final deterministic aggregates above — never changes a number;
   │   falls back to the local report builder's text on any failure
   ▼
Final Report + Trust/Calibration Panel ── GET /api/storm/{id}/report → apps/web report page
```

## Two engines, two separate knobs

PersonaStorm has two independent LLM swap points, controlled by two separate
env vars — do not conflate them:

1. **Persona reaction engine** (`INFERENCE_PROVIDER=mock|nvidia|vllm`,
   `services/inference/`) — generates the 1,000 individual persona reactions
   that make up the swarm. `mock` (deterministic, P0 default) is also the
   permanent CI/eval provider; `vllm` targets a future AMD MI300X/ROCm
   batched swarm; `nvidia` (NVIDIA NIM GLM-5.2) is available today as an
   optional real-LLM swarm path.
2. **Analyst/report engine** (`ANALYST_PROVIDER=mock|nvidia`,
   `services/analyst/`) — a single call per storm that re-narrates the
   executive summary, recommendations, top-objection labels, and kill quote
   from the aggregates the deterministic engine already computed. `mock` is
   the local deterministic report builder's own text (default, no key
   needed); `nvidia` calls NVIDIA NIM GLM-5.2 and, on any failure (missing
   key, network error, invalid JSON), falls back to the local builder's text
   with a note in `quality.notes` — a storm can never fail because of the
   analyst.

In short: **the reaction provider generates the 1,000 persona reactions; the
analyst model summarizes and diagnoses the swarm output — it never invents
the numbers.**
...
| Inference abstraction | `apps/api/app/services/inference` | THE reaction-swarm swap point (`mock_provider.py`, `nvidia_provider.py`, `vllm_provider.py`, `llm_common.py`, `prompts.py`) |
...
| Analyst abstraction | `apps/api/app/services/analyst` | THE report-narration swap point (`mock_analyst.py`, `nvidia_analyst.py`, `prompts.py`, `factory.py`) — re-narrates text only, graceful fallback to `mock_analyst.py` |
...
The analyst model (`ANALYST_PROVIDER=nvidia`, NVIDIA NIM GLM-5.2) only ever
**narrates** aggregates computed in Python — adoption counts, cluster shares,
curve points. Numbers are never generated by a language model. This keeps the
report auditable: every figure traces to `data/runs/{id}.json`. The same rule
applies one layer down: `market_fit_score` is always the output of
`compute_market_fit` — on the mock path the provider calls it directly, and on
the LLM reaction path (`parse_llm_reaction`) the server recomputes it from the
model's raw criteria scores rather than trusting any total the model emits.
